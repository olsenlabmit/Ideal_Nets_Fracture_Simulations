...
def readLAMMPS_restart_triclinic(filename, vflag):##,frac_weak):
   directory = './'##+str(int(100*frac_weak))+'/'
   file_path = os.path.join(directory, filename)
   if not os.path.isdir(directory):
      os.mkdir(directory)
   ...
   line11 = f1.readline()
   line11 = line11.strip()
   xy=float(line11.split(" ")[0])
   xz=float(line11.split(" ")[1])
   yz=float(line11.split(" ")[2])
   ...
   links_unsort = np.genfromtxt(file_path, usecols=(0,3,4,5), skip_header=18, max_rows=n_links)
...
#cnt,ctype,1,conn1,conn2
   if(vflag==0):
      chains[:,0:4] = np.genfromtxt(file_path,usecols=(0,1,2,3), skip_header=18+n_links+3, max_rows=n_chains)
   elif(vflag==1):
      chains[:,0:4] = np.genfromtxt(file_path,usecols=(0,1,2,3), skip_header=18+2*n_links+2*3, max_rows=n_chains)
   else:
      print("Invalid Velocity Flag")
##   print(chains)
   directory = './'##+str(int(100*frac_weak))+'/'
   ...
   return xlo, xhi, ylo, yhi, zlo, zhi, xy, xz, yz, n_links, n_chains, links, chains, atom_types, bond_types, mass, loop_atoms
...
#cnt,ctype,1,conn1,conn2
   if(vflag==0):
      chains[:,0:4] = np.genfromtxt(file_path,usecols=(0,1,2,3), skip_header=17+n_links+3, max_rows=n_chains)
   elif(vflag==1):
      chains[:,0:4] = np.genfromtxt(file_path,usecols=(0,1,2,3), skip_header=17+2*n_links+2*3, max_rows=n_chains)
   else:
      print("Invalid Velocity Flag")
##   print(chains)
   directory = './'##+str(int(100*frac_weak))+'/'
...
##       stop#- ifrst change the following line according to format
       file1.write("{:4} {:4} {:4} {:4}\n".format(cnt,chains[i,1],chains[i,2], chains[i,3]))
...
def writeLAMMPS_triclinic(filename, xlo, xhi, ylo, yhi, zlo, zhi, xy, xz, yz, links, chains, atom_types, bond_types, mass, loop_atoms):
   ...
   file1.write("%7.4f %7.4f %s %s\n" % (ylo,yhi,"ylo","yhi"))
   file1.write("%7.4f %7.4f %s %s\n" % (zlo,zhi,"zlo","zhi"))
   file1.write("%7.4f %7.4f %7.4f %s %s %s\n" % (xy, xz, yz, "xy", "xz", "yz"))
...
#               file1.write("{:4} {:4} {:4} {:4}\n".format(cnt,1,chains[i,1], chains[i,2]))
   ...
   for i in range(0, n_bonds):
       cnt = cnt + 1
       file1.write("{:4} {:4} {:4} {:4}\n".format(cnt,chains[i,0],chains[i,1], chains[i,2]))## cnt,ctype,1,con1,conn2
